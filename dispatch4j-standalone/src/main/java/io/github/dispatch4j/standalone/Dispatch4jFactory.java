...
public final class Dispatch4jFactory {
   ...
   *
   * <p>Scans the specified packages for handler annotations and registers all found handlers. Uses
   * the {@link java.util.concurrent.ForkJoinPool#commonPool()} for asynchronous operations.
   *
   ...
   *
   * <p>Scans the specified packages for handler annotations and registers all found handlers. Uses
   * the provided executor for asynchronous operations, or the default if null is passed.
   *
  ...
  private static <T> void registerMethodHandlersForClass(
    ...
    var handlerInstance = createHandlerInstance(classInfo, "method " + handlerType + " handlers");
  ...
  //                .forEach(classInfo -> registerInterfaceHandlerForClass(classInfo, dispatcher));
  //
